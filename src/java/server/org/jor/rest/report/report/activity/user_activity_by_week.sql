...
(
    SELECT DISTINCT e.member_id AS member_id,
           mt.iso_year AS member_year, mt.iso_week AS member_week,
           ((e.event_time - e.member_created_at) / (7 * 24 * 3600)) AS activity_week
    FROM events e
    ...
    GROUP BY e.member_id, mt.iso_year, mt.iso_week, ((e.event_time - e.member_created_at) / (7 * 24 * 3600))
) a
